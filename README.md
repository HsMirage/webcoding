...
- **独立 Agent 设置** — 各 Agent 拥有独立的默认行为与运行时适配，保持贴近各自原生 CLI 的使用方式
- **Pi 双向 RPC** — 持久连接 Pi 会话，支持真实扩展选择/确认/输入、原生中断、模型发现和命令发现
- **多会话管理** — 创建、切换、重命名、删除会话，删除时同步清除本地 Claude 历史记录
- **会话续接** — 基于 `--resume` 实现跨消息上下文保持，也可通过 SSH 使用 `tmux attach -t claude` 命令加入会话
- **本地历史导入** — Claude 可导入配置目录中的 `projects/` 会话；Codex 可导入 `CODEX_HOME/sessions/` rollout 历史
...
- **Codex 原生审查** — `/review` 直接调用 `codex exec review --uncommitted`，命令后的文字会作为审查要求
...
```
浏览器 ←WebSocket→ Node.js (server.js) ─┬─文件 I/O→ Claude / Codex CLI (detached)
                                       └─JSONL RPC→ Pi CLI (persistent)
```

- Claude / Codex 每轮使用 detached 子进程，stdin/stdout/stderr 通过 `sessions/{id}-run/` 文件传递
- Claude / Codex PID 会持久化，服务重启后通过 `recoverProcesses()` 恢复
- Pi 默认使用持续运行的 `--mode rpc` 进程，同一 Web 会话跨轮复用 stdin/stdout JSONL 通道
- Pi RPC 支持扩展 UI 的双向请求；默认空闲 30 分钟后自动释放，最多并发 8 个运行时，可通过环境变量调整
- 服务重启会结束正在生成的 Pi RPC 轮次，但 Pi 会话文件会保留并在下一轮重新续接
- 设置 `CC_WEB_PI_TRANSPORT=headless` 可恢复 `pi -p --mode json` 的单轮后台运行方式
- 三套 Agent 的启动参数与事件解析由 `lib/agent-runtime.js` 管理，Pi RPC 帧与请求关联由 `lib/pi-rpc-client.js` 管理
