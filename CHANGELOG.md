...
- marked 与 highlight.js 改为内置到 `public/vendor/`，不再依赖 cdnjs：内网、气隙、弱网或 CDN 被墙时不会再出现登录后白屏
- 此前 `app.js` 顶层直接构造 `marked.Renderer()`，CDN 不可达会抛 `ReferenceError` 并中断整个前端初始化；现统一通过守卫引用，任一库缺失时降级为纯文本渲染而非整体崩溃
- CSP 收紧为 `script-src 'self'`，同时移除从未使用的 Google Fonts 来源

### 加载性能

- 静态资源新增 gzip 压缩与协商：首次加载由 798 KB 降至 194 KB
- 版本化资源改用内容指纹（sha256 前 16 位）并按 `immutable` 缓存一年；重复访问返回 304、零字节
- 版本号由 mtime 改为内容指纹，避免 `rsync -t`、tar 解包等保留时间戳的部署方式导致版本不变、客户端长期使用旧资源
- 仅当请求的 `?v=` 与实际内容指纹一致时才授予 `immutable`，防止任意 URL 在共享缓存中长期占位
- gzip 与未压缩表示使用不同 ETag；补全 `If-None-Match: *`、弱比较与 `Accept-Encoding: *` 处理

### 运行时性能

- 会话列表由「每次读取并解析全部会话文件」改为按 (mtime, size) 键控的逐文件摘要缓存：86 个会话 / 14 MB 场景下由 33 ms 阻塞降至 0.27 ms，不再因每次会话更新反复阻塞事件循环、拖慢所有客户端的流式输出
- `saveSession()` 额外按 id 主动失效摘要，覆盖 FAT/SMB/NFS 等低精度时间戳文件系统上的等长改写
- 流式渲染的语法高亮改为记忆化：108 次渲染 tick / 18 个代码块场景下由 74 ms 降至 7 ms，长回答与移动端明显更流畅
- 静态资源缓存与高亮缓存均改为同时限制条目数与总字节数，避免大文件或大代码块占用过量内存
...
- 在“默认模型”旁新增“获取模型列表”，可使用尚未保存的提供商配置实时读取模型；已有配置即使 API Key 显示为掩码，也会安全复用后端保存的真实密钥
- 模型列表可直接回填默认模型，并在上游返回元数据时自动填入上下文长度；列表仅用于选择，不会作为固定模型清单写入配置
- 新增提供商级上下文长度，分别适配 Claude 的 `CLAUDE_CODE_MAX_CONTEXT_TOKENS`、Codex 的 `model_context_window` 和 Pi 的 `contextWindow`
- Claude 自定义渠道默认以模型名 `[1m]` 后缀启用 1M 上下文模式；共享配置仍保存原始模型 ID，后缀不会传入 Codex 或 Pi

### 模型来源

- 会话内 `/model` 明确区分模型来源：本地渠道读取各 Agent 的真实本地配置，自定义渠道实时请求当前选中的 AI 提供商
- 模型选择器显示当前来源，方便确认正在使用本地配置还是具体提供商
- 从自定义渠道切回本地渠道后，不再请求之前的提供商模型接口

### 验证

- 49 项隔离回归全部通过
- 覆盖新建提供商、掩码密钥、上下文长度落盘、Claude `[1m]` 隔离，以及自定义与本地渠道切换
- 已完成桌面端浏览器实测，确认按钮布局、模型列表展开、默认模型与上下文长度回填正常

## v2.0.8 - 2026-07-15

### Windows 后台持久化

- 修复 PowerShell 5.1 将以反斜杠结尾的带引号 `InstallDir` 参数解析为含多余引号的非法路径，导致服务脚本在注册计划任务前退出的问题
- `webcoding.cmd` 与 `start.bat` 改用不会让反斜杠紧贴结束引号的目录参数，更新后立即重启和后续管理命令使用相同兼容方式
- 安装器与服务脚本统一清理路径首尾空白、多余引号和末尾目录分隔符，同时为旧启动器留下兼容处理
- Windows 计划任务启用“错过触发后尽快启动”，提高用户登录后自动恢复运行的可靠性
- 当前用户免管理员安装仍采用“用户登录后自动运行”，不承诺登录前以系统服务方式启动

### 验证

- 49 项隔离回归全部通过
- 新增 PowerShell 5.1 路径参数、启动器末尾反斜杠和计划任务补偿启动的回归覆盖
